...
public class WelcomeEmailConsumer {
    ...
    @RabbitListener(queues = {"${rabbitmq.email.queues.welcome.name}"})
    public void handleWelcomeEmail(EmailPayload payload){
        try {
            ...
            log.info("Welcome email sent to {}", payload.getTo());

        } catch (MessagingException | java.io.UnsupportedEncodingException e) {
            log.error("Failed to send welcome email to {}: {}", payload.getTo(), e.getMessage());
